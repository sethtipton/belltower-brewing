...
const readDebugFlag = (): boolean => {
  ...
  const win = globalThis as {
    BT_PARKING_GUIDE_CONFIG?: { debug?: boolean };
  };
  if (typeof win.BT_PARKING_GUIDE_CONFIG?.debug === 'boolean') {
    return Boolean(win.BT_PARKING_GUIDE_CONFIG.debug);
  }
  return readStorageFlag('bt_parking_debug') || readStorageFlag('bt_pairing_debug');
};

const getTraceId = (): string => {
  if (typeof globalThis === 'undefined') return 'server';
  const win = globalThis as { sessionStorage?: Storage };
  const key = 'bt_parking_trace_id';
  try {
...
const formatPrefix = (scope: string, event: string): string => `[PARKING_GUIDE] ${scope} ${event}`;

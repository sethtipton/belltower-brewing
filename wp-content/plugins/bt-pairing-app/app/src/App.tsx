...
const isAdmin = Boolean(
  pairingGlobals?.BT_PAIRING_APP_CONFIG?.isAdmin
...
const getServerCacheHash = (): string => {
  const hash = pairingGlobals?.BT_PAIRING_APP_CONFIG?.cacheHash
    ?? pairingGlobals?.PAIRING_APP?.cacheHash
    ?? pairingGlobals?.PAIRINGAPP?.cacheHash;
...
function extractSuccessMessage(raw: PairingResponse | null): string {
  ...
  const direct = root && typeof root.success_message === 'string' ? root.success_message : '';
  if (direct) return direct;
  const result = root && isRecord(root.result) ? root.result : null;
  if (result && typeof result.success_message === 'string') return result.success_message;
  if (result && typeof result.successMessage === 'string') return result.successMessage;
  return '';
}
...
function AppContent(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    log.info('boot.dataSources', {
      phase: 'boot',
      ...sources,
      hasGlobals: Boolean(pairingGlobals?.BT_PAIRING_APP_CONFIG ?? pairingGlobals?.PAIRING_APP ?? pairingGlobals?.PAIRINGAPP),
  ...
  const handleSubmit = async (preparedOverride?: PreparedAnswers | null): Promise<void> => {
    ...
    try {
      ...
      if (matches.length) {
        ...
        const aiMessage = extractSuccessMessage(fetched);
        console.log('[pairing-app] success_message', aiMessage);
        setSuccessMessage(
          aiMessage ||
          `We ranked the beers below by how well they match your answers and highlighted ${matches.length} recommendations.`
        );
        setSuccessIsAi(Boolean(aiMessage));
        setTopRecommendationName(matches[0]?.beer?.name ? String(matches[0].beer.name) : '');
